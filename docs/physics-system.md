...
- **`PhysicsLayerManager` is process-global and static.** `InitDefaults` clears and rebuilds the layer set; call it once (from `PhysicsSystem::Init`). The Jolt filter interfaces read it live, and the broadphase layer count is snapshotted in `JoltBroadPhaseLayerInterface`'s ctor — add layers before creating a scene.
