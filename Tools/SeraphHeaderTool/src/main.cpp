...
// leaving the file (and its mtime) untouched on a match. Every gen output is
// wired with `restat = 1`, so an unchanged .gen.cpp lets Ninja skip recompiling
// it and relinking downstream. This matters because a forced CMake reconfigure
// (armed by the CONFIGURE_DEPENDS glob) reruns every gen step on each build even
// when nothing changed — without this, each rerun would bump the mtime and drag
// the whole recompile+link chain along. Returns false only on a write error.
...
{
    {
        ...
        if (in)
        {
            ...
            if (existing == content)
                return true; // unchanged — do not touch the file
        }
    }
...
void EmitProperty(std::ostream& os, const TypeInfo& t, const PropertyInfo& p)
{
    if (p.IsAccessor())
          ...
          const std::string& includePath)
{
    ...
    if (!WriteIfDifferent(outPath, os.str()))
        return false;
                  ...
                  const std::string& outPath, const std::string& mainHeader)
{
    ...
    clang_getInclusions(
        ...
        {
            // Emit the REAL (canonicalized, symlink-resolved) path. libclang's
            // clang_getFileName hands back the path exactly as the header was
            // found on the search path, which for toolchain headers can be a
            // non-canonical spelling like
            //   /../lib64/gcc/x86_64-.../16/../../../../include/c++/16/array
            // — note the leading "/.." (the search-root prefix is lost). That
            // normalizes to "/include/c++/16/array", which DOES NOT EXIST. Ninja
            // records it as a prerequisite, never finds it on disk, and therefore
            // treats the gen step as dirty on EVERY build — so every annotated
            // header re-runs the (expensive) libclang parse each build. The
            // canonical path resolves to the real "/usr/include/..." file, which
            // exists, so the dependency is satisfied and the step stays cached.
            // Fall back to the raw spelling only if realpath is unavailable.
            std::string p = Str(clang_File_tryGetRealPathName(included));
            if (p.empty())
                p = Str(clang_getFileName(included));
            static_cast<std::vector<std::string>*>(cd)->push_back(std::move(p));
        },
        &files);
    ...
    for (const std::string& f : files)
        os << " \\\n  " << escape(f);
    os << "\n";
    WriteIfDifferent(depPath, os.str());
}
...
// Cheap text pre-scan (NO libclang): does this header actually USE an annotation
// macro (SCLASS/SPROPERTY/SENUM/SFUNCTION followed by '(')? Mirrors the CMake
// content filter that sht_reflect_glob used to run at configure time, moved here
// so the build wires a gen step for EVERY header but only annotated ones pay the
// libclang parse cost. A macro on a '#define'/'#include' line (i.e. Annotations.h
// itself) is not a use. This must stay in sync with the identical fast path so a
// header that gains its first annotation is picked up by a plain build.
bool HasAnnotationMacro(const std::string& path)
{
    std::ifstream in(path);
    if (!in)
        return true; // can't read it — be safe, let libclang try (and report)
    static const char* kMacros[] = {"SCLASS", "SPROPERTY", "SENUM", "SFUNCTION"};
    std::string line;
    while (std::getline(in, line))
    {
        // Skip preprocessor directives (#define SCLASS(...) / #include lines).
        std::size_t first = line.find_first_not_of(" \t");
        if (first != std::string::npos && line[first] == '#')
            continue;
        for (const char* m : kMacros)
        {
            std::size_t pos = line.find(m);
            while (pos != std::string::npos)
            {
                std::size_t after = pos + std::strlen(m);
                // Must be a whole-token match followed (after optional space) by '('.
                bool wordStart = (pos == 0) || (!std::isalnum((unsigned char) line[pos - 1])
                                                && line[pos - 1] != '_');
                std::size_t p = after;
                while (p < line.size() && (line[p] == ' ' || line[p] == '\t'))
                    ++p;
                if (wordStart && p < line.size() && line[p] == '(')
                    return true;
                pos = line.find(m, after);
            }
        }
    }
    return false;
...
// Write a .gen.cpp for a header with no annotations: a header comment only, no
// registrations. Keeps the build's source list stable (the file always exists)
// without invoking libclang.
bool WriteEmptyGen(const std::string& outPath, const std::string& includePath)
{
    std::ostringstream os;
    os << "// Generated by SeraphHeaderTool — no reflection annotations in\n"
       << "// " << includePath << "\n";
    return WriteIfDifferent(outPath, os.str());
}

// Write a trivial depfile: the output depends only on the header itself, so a
// later edit that ADDS an annotation reruns the tool (which then parses it).
void WriteTrivialDepfile(const std::string& depPath, const std::string& outPath,
...
int main(int argc, char** argv)
{
    ...
    // Fast path (emit mode only): if the header uses no annotation macro, skip
    // libclang entirely and emit an empty .gen.cpp + trivial depfile. This lets
    // the build wire a gen step for EVERY header (so a newly-annotated header is
    // picked up by a plain build, no reconfigure) while only annotated headers pay
    // the parse cost. Dump mode (no -o) always parses.
    if (!outPath.empty() && !HasAnnotationMacro(header))
    {
        if (!WriteEmptyGen(outPath, includePath))
            return 1;
        if (!depPath.empty())
            WriteTrivialDepfile(depPath, outPath, header);
        return 0;
    }
