...
void EditorCamera::OnUpdate(const f64 ts)
{
    const float tsMs = static_cast<float>(ts) * 1000.0f;
    ...
    if (Input::IsMouseButtonDown(Mouse::Right) && !Input::IsKeyDown(Key::LAlt))
    {
        ...
        if (Input::IsKeyDown(Key::Q))
            m_PositionDelta -= tsMs * speed * glm::vec3{ 0.f, yawSign, 0.f };
        if (Input::IsKeyDown(Key::E))
            m_PositionDelta += tsMs * speed * glm::vec3{ 0.f, yawSign, 0.f };
        if (Input::IsKeyDown(Key::S))
            m_PositionDelta -= tsMs * speed * m_Direction;
        if (Input::IsKeyDown(Key::W))
            m_PositionDelta += tsMs * speed * m_Direction;
        if (Input::IsKeyDown(Key::A))
            m_PositionDelta -= tsMs * speed * m_RightDirection;
    ...
    else if (Input::IsKeyDown(Key::LAlt))
    {
        ...
        if (Input::IsMouseButtonDown(Mouse::Middle))
        {
            DisableMouse();
            MousePan(delta);
        }
        ...
        else if (Input::IsMouseButtonDown(Mouse::Right))
        {
            DisableMouse();
...
{
    const float x = glm::min(float(m_ViewportRight - m_ViewportLeft) / 1000.0f, 2.4f);
    const float xFactor = 0.0366f * (x * x) - 0.1778f * x + 0.3021f;

    const float y = glm::min(float(m_ViewportBottom - m_ViewportTop) / 1000.0f, 2.4f);
