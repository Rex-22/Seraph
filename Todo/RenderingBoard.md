...
**Startup uniform churn — FIXED.** Initially the embedded `tonemap` program rebuilt every frame during the pre-project startup window (~180–245 uniform create/recycle cycles), because `AssetManager::AddMemoryAsset` no-ops before a project's AssetManager is installed, so the built `ShaderAsset` (and its bgfx program + reflected uniforms) was dropped and rebuilt each frame. Fixed by having `ShaderManager` **own** built embedded `ShaderAsset`s in a static cache (new `ShaderManager::GetProgram(name)`), so the program is stable regardless of AssetManager state; `Renderer::ResolveTonemapProgram` now uses it. Also benefits `DebugRenderer` (same pattern). Verified: `u_tonemapParams`/`s_hdr` now created exactly once. (`ShaderManager.{h,cpp}`, `Renderer.cpp`.)
